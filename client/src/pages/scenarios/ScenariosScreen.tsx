...
const HistoricalEventsOverlay: React.FC<{
  ...
  onClose: () => void;
  onApply: () => void;
}> = ({ visible, overlays, onToggleOverlay, onClose, onApply }) => {
    ...
    >
      <View style={styles.modalOverlay}>
        <View style={styles.eventsOverlayContainer}>
          ...
          >
            <Text style={styles.applyButtonText}>Apply Overlays</Text>
          </TouchableOpacity>
        </View>
      </View>
    </Modal>
...
const ScenariosScreen: React.FC = () => {
  ...
  const [showOverlaysOnChart, setShowOverlaysOnChart] = useState(false);
  ...
  // Handle applying historical overlays to chart
  const handleApplyHistoricalOverlays = () => {
    const hasVisibleOverlays = historicalOverlays.some(overlay => overlay.visible);
    setShowOverlaysOnChart(hasVisibleOverlays);
    
    if (hasVisibleOverlays) {
      const overlayCount = historicalOverlays.filter(overlay => overlay.visible).length;
      Alert.alert(
        'Overlays Applied',
        `${overlayCount} historical event${overlayCount > 1 ? 's' : ''} now visible on chart`,
        [{ text: 'OK' }]
      );
    } else {
      Alert.alert(
        'No Overlays Selected',
        'Please select at least one historical event to display',
        [{ text: 'OK' }]
      );
    }
  };
    ...
    return (
      <View style={styles.container}>
        ...
        <ScrollView style={styles.detailsContainer}>
          ...
          <View style={styles.detailsCard}>
            ...
            {Object.keys(selectedRun.assetClassImpacts).length > 0 ? (
              ...
              <Text style={styles.noDataText}>No asset class breakdown data available</Text>
            )}
          </View>
          ...
          {/* Scenario Impact Chart with Historical Overlays */}
          <ScenarioImpactChart
            scenarioImpact={selectedRun.impact}
            scenarioName={selectedRun.scenarioName}
            historicalOverlays={historicalOverlays}
            showOverlays={showOverlaysOnChart}
            onToggleOverlaysVisibility={() => setShowOverlaysOnChart(!showOverlaysOnChart)}
            onConfigureOverlays={() => setShowHistoricalOverlay(true)}
          />
        ...
        <HistoricalEventsOverlay
          ...
          onApply={handleApplyHistoricalOverlays}
        />
